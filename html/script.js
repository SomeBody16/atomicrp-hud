...
const mapRadius = stage.height() * 0.42;
const mapPos = {
  x: mapRadius + (stage.width() * 0.04),
  y: (stage.height() * 0.975) - mapRadius,
}
const statsRadius = mapRadius + 50;
const textRadius = statsRadius + 50;
...
const getStatPosition = (order) => getCirclePos(statsRadius, 235 - (order * 13), mapPos);
const initStats = () => {
  const stats = {
    health: {
      order: 0,
  ...
  for (const statName in stats) {
    ...
    const icon = new Konva.Path({
      ...
      fill: 'lightgray',
      scale: {
        x: 0.06 * (stat.iconScale ?? 1),
        y: 0.06 * (stat.iconScale ?? 1),
      },
    });
...
const initPlace = () => {
  const primary = new Konva.Text({
    ...getCirclePos(textRadius, 153, mapPos),
    text: `Pillbox Hill`,
    fontSize: 40,
    fontFamily: 'Redemption',
    fill: '#F60',
  ...
  const secondary = new Konva.Text({
    ...getCirclePos(textRadius, 142, mapPos),
    text: `Elgin Ave`,
    fontSize: 30,
    fontFamily: 'Redemption',
    fill: '#DDD',
...
const hudManager = {
  ...
  car: {
    update: (speed, turnover, speedText) => {
      new Konva.Tween({node: hud.component.car.speedometer, duration: 0.1, angle: 360 - (speed * 100)}).play();
      new Konva.Tween({node: hud.component.car.tachometer, duration: 0.1, angle: 360 - (turnover * 100)}).play();
